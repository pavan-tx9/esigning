...
| `CONSENT_SPAN_SECONDS` | `0` | for how long an acceptance of the ESIGN disclosure also stands for the same person's other documents on the same host (one sitting at one desk). `0`: consent is collected once per envelope, which is what the base spec describes. At most `3600`, which verification re-checks. Every envelope still records its own acceptance; what changes is whether the notice was displayed again, and the trail, the certificate and `esign verify` all say which. Never for a kiosk session. Off unless compliance has agreed |
