...
them: 1,964 backend tests, 142 frontend tests and 23 demo-host tests at the time of writing, plus the two Playwright suites (14 specs against the mocks, 12 against the real stack through the demo host) that `make check` does not run.
...
| "…and who filed it and attested to it" | `archive.created` (actor: the host) then `archive.attested` (actor: the staff member, by opaque id, role `staff`) are the first two events; the mutable `attestation` column is compared against `archive.attested` before sealing and again by verification | `archives/service.py`; `verification/__init__.py::_check_archive_row_against_trail` | `tests/archives/test_filing.py::test_the_trail_is_the_one_the_addendum_describes`; `tests/archives/test_verification.py::test_a_rewritten_attestation_is_caught`, `::test_a_tampered_archive_audit_row_is_caught` |
...
| "capped at 900 seconds by validation" | `config.py`, `le=900` | ⚠️ **No test asserts the cap.** The bound is one Pydantic constraint, and the suite exercises the span *at* 900 (`tests/reauth_span/test_signing_queue.py::test_an_attestation_older_than_the_maximum_age_is_refused_inside_the_span`) but never asserts that 901 is refused at startup. Everything else in this table is proved by a test that would fail if the behaviour were removed; this one is not. See §6 G11 |
...
| G11 | **The 900-second cap on `REAUTH_SPAN_SECONDS` has no test.** The bound is a Pydantic constraint (`Field(default=0, ge=0, le=900)`) and it does hold — a settings object built with 901 raises — but nothing in the suite asserts it, so the claim in §2.11 C rests on reading the code rather than on a test that would fail if the constraint were dropped | Low. Every other containment of the span is tested, and the cap is the *outer* bound: the inner one (`REAUTH_MAX_AGE_SECONDS`) is enforced on every resolution and is thoroughly tested. But it is the number compliance will be quoted, and an accidental widening would be silent | One test in `tests/foundation/test_clock_ids_config.py` asserting that `Settings(reauth_span_seconds=901)` is refused and that 900 is accepted |

| G12 | **The `adopted_signatures` write-once rule has no direct test.** Migration `0700` revokes `DELETE`/`TRUNCATE` from `esign_app` and adds a trigger that permits exactly one UPDATE (the revocation) and refuses everything else; both work (checked by hand against a live database: the app role gets "permission denied", the owner role gets "DELETE on adopted_signatures is forbidden"). But unlike the other append-only tables — which have `tests/foundation/test_roles.py`, `tests/audit/test_roles.py` and `tests/storage/test_roles.py` attacking them directly — this one is only exercised through the API (`tests/adopted_signatures/test_revoking.py::test_a_revoked_row_stays`, `tests/adopted_signatures/test_racing.py`) | Low. The rows are secondary evidence: the digest of what was stamped is in the hash-chained trail, so a removed row is a *finding* (`captures_match_trail`) rather than a silent loss | Extend `tests/foundation/test_roles.py` with the same treatment the other evidence tables get: app-role `DELETE` refused, owner-role `DELETE` and `TRUNCATE` refused, a second UPDATE of a revoked row refused, and an UPDATE that changes any other column refused |
