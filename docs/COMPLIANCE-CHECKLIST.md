...
| DON'T silently re-apply a saved signature | ✅ | Nothing is persisted between sessions. The draft lives in memory for one session only | `frontend/src/flow/draft.test.ts`; `frontend/src/App.test.tsx` "a kiosk session ends on hand-back with every trace of the patient gone" |
...
| C3 | **Approve `APPROVED_DOCUMENT_TYPES`.** Today: `patient_consent`, `hipaa_acknowledgement`, `procedure_consent` | Compliance | The guide is explicit that engineers do not decide which document types may be signed electronically |
