...
| Area | What it does |
|---|---|
| **Signing UI** | Embeddable, accessible, patient-friendly. Three screens — read and agree, sign, done — and three taps per document for a clinician working a queue with a saved signature. Draw, type or click to sign. Works on a phone or a shared clinic tablet. |
...
| **Intent and consent** | Versioned ESIGN disclosure under the last page, explicit acceptance, every page viewed before the sign button unlocks, one press that says what it does and is the intent to sign, a decline-to-paper path that is always visible. |
...
| **Also** | Paper-signed scans archived with a staff attestation and sealed; saved signatures; a clinician signing queue whose documents open one after another; multi-signer envelopes in sequence or parallel. |
...
| `priya` | The signing queue: confirm identity once, agree to sign electronically once, and sign five orders in a row — three taps each after that, opening one after another. |
| `priya` → **Reports** | Thirty pages of one patient's own record, generated by the EHR and signed as the document itself — no template exists for it. The case review beside it is co-signed by `tomas`. |
...
An envelope starts from one of two things. A **published template** plus that patient's chart data
covers the forms everybody signs — consents, acknowledgements, orders — and the wording is reviewed
once, on the template. A **document you generated** is posted to the same route as multipart when no
template could express it: a per-patient report, twenty or thirty pages of their own record,
different every time, with a signature block at the end. From revision 1 onwards the two are the
same product — same review gate, same consent, same seal, same verification — and only the
provenance differs, which the audit trail and the certificate of completion both say in as many
words.
...
|---|---|
| [docs/INTEGRATION.md](docs/INTEGRATION.md) | Integrate an EHR: API calls, embedding, re-authentication, webhooks, documents you generated yourself, paper archives, saved signatures, the signing queue. |
...
| [docs/SPEC.md](docs/SPEC.md) | The architecture and the rules it is built from. Addenda: [paper archives, saved signatures, signing queue](docs/SPEC-ADDENDUM-1.md), [host-supplied documents](docs/SPEC-ADDENDUM-2.md), [a shorter signing flow, queue auto-advance, consent once per run](docs/SPEC-ADDENDUM-3.md). |
